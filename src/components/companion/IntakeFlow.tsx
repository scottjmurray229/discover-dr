...
const SAMPLE_PASTE = `Day 1 - Arrive Punta Cana, resort check-in, beach afternoon
Day 2 - Saona Island catamaran excursion, natural pool
Day 3 - Drive to Santo Domingo, Zona Colonial walking tour
Day 4 - Los Tres Ojos caves, Alcazar de Colon, Malecon sunset
Day 5 - Drive to Samana, Las Terrenas beach, El Limon waterfall
Day 6 - Whale watching (Jan-Mar) or Los Haitises National Park
Day 7 - Fly home from Samana or transfer to Punta Cana`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Dominican Republic itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: COLORS.deepNight, lineHeight: 1.6, fontStyle: 'italic' }}>
          "We had zero signal in the mountains near Jarabacoa and the app had everything — directions, restaurant picks, even the right phrases. Felt like traveling with a local friend."
        </div>
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'east-coast') return t.route.match(/Punta Cana|La Romana/i);
          if (filter === 'north-coast') return t.route.match(/Puerto Plata|Cabarete/i);
          if (filter === 'interior') return t.route.match(/Jarabacoa|Constanza|Barahona/i);
          return true;
        });
    ...
    return (
      <>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto' as const }}>
          {['all', 'east-coast', 'north-coast', 'interior'].map((f) => (
    ...
    <>
      ...
      {!apiError && (
        <>
          ...
          {[
            { icon: '🧭', title: 'Real Directions', desc: "Not Google Maps pins — actual 'guagua to Zona Colonial, get off at Parque Colon' directions" },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '💬', title: 'Local Phrases', desc: 'Dominican Spanish phrases tuned to your destinations with pronunciation' },
            ...
            { icon: '💵', title: 'Tipping Guide', desc: 'What to tip in every Dominican Republic situation, from trike drivers to dive masters' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
