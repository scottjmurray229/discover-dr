 ...
 *   2. Create project "Discover Dominican Republic"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Dominican Republic Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'dr'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Dominican Republic ${year}`,
      description: `${title}\n\nFull guide: https://discoverdr.info/${slug}/`,
      tags: [`Dominican Republic ${slug}`, 'Dominican Republic travel', `Dominican Republic ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Dominican Republic ✈️ #shorts`,
      description: `${title} — Full guide: https://discoverdr.info/destinations/${slug}/`,
      tags: [`${title} Dominican Republic`, 'Dominican Republic travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
  ...
  return {
    ...
    description: '',
    tags: ['Dominican Republic travel'],
    categoryId: '19',
    privacyStatus: 'private',
  };
}
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Dominican Republic — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Dominican Republic for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoverdr.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoverdr.info/companion/

#DominicanRepublic #${title.replace(/\s+/g, '')} #DominicanRepublicTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  const tags = [
    ...
    `${title} travel guide`,
    `Dominican Republic ${new Date().getFullYear()}`,
    `${title} things to do`,
    `where to stay ${title}`,
    'Dominican Republic travel',
    'Dominican Republic food',
    'Dominican Republic budget travel',
    'Dominican Republic tips',
  ];
  if (dest?.region) tags.push(`${dest.region} Dominican Republic`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER DOMINICAN REPUBLIC     ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
